...
Project Chrome                    ← UI-02 Completed
  ├── Обзор                       ← UI-10 Completed (`b0c9659`; группа I: passport-about + stage track UI-04)
...
### Группа I — Project Overview / паспорт-инфографика — Completed via UI-10

| | |
|---|---|
| **Уже выполнено** | stage track из `stage_model` (UI-04); passport-about + целостный обзор (UI-10 @ `b0c9659`) |
...
## 4a. UI-10 — Project Overview / целостный обзор проекта — Completed

| | |
|---|---|
| **Статус** | **Completed** |
| **Реализация** | `b0c9659` — `feat(ui): UI-10 — добавить целостный обзор проекта` |
| **Источник scope** | группа **I** (закрыта через UI-10) |
| **Зависимости** | UI-02 Chrome, UI-04 stage track (потребление), passport Bridge UI-03/06a |
| **Итоговая приёмка** | **PASS**, **9/9** сценариев на `b0c9659` |
...
- Укорочение отображения полного абсолютного пути папки в Обзоре (см. backlog/UX ниже) — **не** дефект и **не** scope UI-10.

### Критерии результата UI-10 — все закрыты

1. ~~Раздел «Обзор» открывается и показывает **целостный обзор проекта**, а не экран текущей задачи.~~ **Закрыт**
2. ~~Видны ключевые сведения о проекте понятным языком (название, идея/цель, аудитория, ожидаемый результат, статус; путь — по-человечески).~~ **Закрыт**
3. ~~Stage track из UI-04 **встроен** в обзор; логика/`stage_model` **не** дублируется и **не** переписывается.~~ **Закрыт**
4. ~~«Текущая задача» остаётся отдельным разделом и не подменяет Обзор.~~ **Закрыт**
5. ~~Demo-изоляция (UI-08) и Core-сценарии Create/Connect/History не ломаются.~~ **Закрыт**
6. ~~Нет блокирующих ошибок консоли; результат предъявлен скриншотами (desktop + телефон).~~ **Закрыт**

### Результат приёмки UI-10

- Итоговая приёмка на `b0c9659`: **PASS**, **9/9** сценариев.
- Проверено: реальный проект (passport), demo без Bridge passport/stage, fallback без passport, разделение Обзор / Текущая задача, единственный `#desktopStageTrack`, history back, desktop + телефон, отсутствие блокирующих ошибок консоли; скриншоты предъявлены.
- Блокирующих дефектов нет.

### Backlog / UX-наблюдение (вне UI-10)

`humanWorkFolderPath` в Обзоре показывает полный абсолютный путь (`D:\…`), а не короткое имя папки. Зафиксировано как **неблокирующее UX-наблюдение** для будущего backlog; **не** входит в закрытие UI-10 и **не** считается дефектом приёмки UI-10.
